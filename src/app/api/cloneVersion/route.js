...
export async function POST(req) {
  ...
  try {
    ...
    console.log(
      "[LOG] Source version exists with version:",
    ...
    const { data: sourcePlanParams, error: sourcePlanParamsError } =
      await supabase
    ...
    if (sourcePlanParamsError && sourcePlanParamsError.code !== "PGRST116") {
      console.error(
        "[ERROR] Error fetching source plan_parameters:",
        sourcePlanParamsError
      );
      return NextResponse.json(
    ...
    if (sourcePlanParams) {
      ...
      console.log(
        "[LOG] Inserting new plan_parameters for new version:",
        newVersionData.id
      );
      ...
      if (insertPlanParamsError) {
        console.error(
          "[ERROR] Error inserting new plan_parameters:",
          insertPlanParamsError
        );
        return NextResponse.json(
    ...
    const getClientId = (entity) => {
      ...
      if (
        ...
        console.warn(
          `[LOG] UUID detected in id (${entity.id}), client_id not provided, falling back to id`
        );
        return entity.id;
      }
      return entity.id;
    ...
    const { data: sourcePoints, error: sourcePointsError } = await supabase
      .from("points")
      .select(
        "id, client_id, x_coordinate, y_coordinate, z_coordinate, snapangle, rotation"
      )
      .eq("version_id", version_id);
    ...
    if (sourcePoints && sourcePoints.length > 0) {
      ...
      sourcePoints.forEach((sourcePoint) => {
        const newPoint = insertedPoints.find(
          (p) => p.client_id === getClientId(sourcePoint)
        );
        if (newPoint) {
          oldToNewPointIdMap.set(sourcePoint.id, newPoint.id);
        } else {
          console.warn(
            `[LOG] No new point found for source point client_id: ${getClientId(
              sourcePoint
            )}`
          );
        }
      ...
      console.log(
        "[LOG] Created point ID mapping with",
        oldToNewPointIdMap.size,
        "entries"
      );

      if (oldToNewPointIdMap.size < sourcePoints.length) {
        console.warn(
          "[LOG] Incomplete point ID mapping, some points may not be referenced correctly"
        );
      }

      // Step 8: Clone walls  ✅ FIX: clone ALL columns safely
      ...
      if (sourceWalls && sourceWalls.length > 0) {
        ...
        // ✅ Copy everything except id/version_id/linestring_geom, then override mapped point ids + new version_id
        const newWallsToInsert = sourceWalls.map((wall) => {
          const {
            id, // drop
            version_id: _oldVersionId, // drop
            linestring_geom, // drop (trigger will regenerate)
            ...rest
          } = wall;
          ...
          return {
            ...rest,
            startpointid:
              oldToNewPointIdMap.get(wall.startpointid) || wall.startpointid,
            endpointid:
              oldToNewPointIdMap.get(wall.endpointid) || wall.endpointid,
            version_id: newVersionData.id,
        ...
        sourceWalls.forEach((sourceWall) => {
          const newWall = insertedWalls.find(
            (w) => w.client_id === getClientId(sourceWall)
          );
          if (newWall) {
            oldToNewWallIdMap.set(sourceWall.id, newWall.id);
          } else {
            console.warn(
              `[LOG] No new wall found for source wall client_id: ${getClientId(
                sourceWall
              )}`
            );
          }
        ...
        console.log(
          "[LOG] Created wall ID mapping with",
          oldToNewWallIdMap.size,
          "entries"
        );

        if (oldToNewWallIdMap.size < sourceWalls.length) {
          console.warn(
            "[LOG] Incomplete wall ID mapping, some walls may not be referenced correctly"
          );
        }

        // Step 9: Clone articles
        console.log("[LOG] Fetching source articles for version:", version_id);
        const { data: sourceArticles, error: sourceArticlesError } =
          await supabase
            .from("articles")
        ...
        if (sourceArticlesError) {
          console.error(
            "[ERROR] Error fetching source articles:",
            sourceArticlesError
          );
          return NextResponse.json(
        ...
        if (sourceArticles && sourceArticles.length > 0) {
          ...
          const newArticlesToInsert = sourceArticles.map((article) => {
            ...
            if (newData.point_ids) {
              newData.point_ids = newData.point_ids.map(
                (oldId) => oldToNewPointIdMap.get(oldId) || oldId
              );
            }
            if (newData.wallId && !validWallIds.has(newData.wallId)) {
              console.warn(
                `[LOG] Invalid wallId ${newData.wallId} for article client_id: ${getClientId(
                  article
                )}, preserving as null`
              );
              newData.wallId = null;
              ...
              newData.wallId =
                oldToNewWallIdMap.get(newData.wallId) || newData.wallId;
            }
            if (newData.referencePointId) {
              newData.referencePointId =
                oldToNewPointIdMap.get(newData.referencePointId) ||
                newData.referencePointId;
            }
            if (newData.lines && newData.lines.startPointId) {
              newData.lines.startPointId =
                oldToNewPointIdMap.get(newData.lines.startPointId) ||
                newData.lines.startPointId;
              newData.lines.endPointId =
                oldToNewPointIdMap.get(newData.lines.endPointId) ||
                newData.lines.endPointId;
            }
          ...
          if (insertArticlesError) {
            console.error(
              "[ERROR] Error inserting new articles:",
              insertArticlesError
            );
            return NextResponse.json(
