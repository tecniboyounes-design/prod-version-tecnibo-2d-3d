...
const ODOO_BASE = process.env.ODOO_BASE;                  // e.g. https://erptest.tecnibo.com
const CLIENT_ID = process.env.ODOO_CLIENT_ID;             // y8Bsx_...
const CLIENT_SECRET = process.env.ODOO_CLIENT_SECRET;     // tL_08R...
...
export async function GET(req) {
  ...
  let tk;
  try { tk = JSON.parse(text); } catch { return new Response('Bad JSON from token endpoint', { status: 400 }); }
  ...
  setCookie(respHeaders, 'odoo_at', accessToken, { maxAge: Math.max(300, expiresIn - 60), secure: isHttps });
  if (refreshToken) setCookie(respHeaders, 'odoo_rt', refreshToken, { maxAge: 60 * 60 * 24 * 30, secure: isHttps });
