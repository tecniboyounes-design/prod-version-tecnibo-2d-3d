...
export async function POST(req) {
    try {
        const { title, description, uid, lang, company_ids, user_companies, projectNumber } = await req.json();
        console.log('project number:', projectNumber);

        const fullDescription = `${description} (Project No: ${projectNumber})`;

        const url = "http://192.168.30.33:8069/web/dataset/call_kw";
        const currentCompanyId = user_companies && user_companies.current_company ? user_companies.current_company : 11; 
        ...
        return new Response(JSON.stringify({ message: 'Error creating project task', error: error.message }), { status: 500 });
    }
}
