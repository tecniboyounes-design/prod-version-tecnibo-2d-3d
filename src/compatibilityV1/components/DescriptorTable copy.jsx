...
const OP_CHOICES = ['AND', 'OR', 'NOT AND', 'NOT OR'];
const COMP_CHOICES = ['=', '<>', '<', '<=', '>', '>=', 'B', '!B', 'E', '!E', 'C', '!C'];

const makeId = (treeIndex, pathArr) => `${treeIndex}|${pathArr.join('/')}`;
...
const normCmp = (v) => normalizeComparisonOp(String(v || '=')); // canonicalize to IMOS token
          ...
          <TableRow key={`${treeIndex}::${id}::comparison`}>
            ...
            <TableCell width={180}>
              <TextField
                ...
                size="small"
                value={op}
                onChange={(e) => handleCompChange(treeIndex, path, 'operator', e.target.value)}
                error={Boolean(rowIssues?.operator)}
                helperText={rowIssues?.operator}
                fullWidth
                disabled={isReadOnly}
              >
                {COMP_CHOICES.map((cop) => (
                  <MenuItem key={cop} value={cop}>
                    {cop}
                  </MenuItem>
                ))}
              </TextField>
